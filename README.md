...
* **Tenant Isolation (RLS)**: Defense-in-depth Postgres Row-Level Security enforced at the database layer — the app runs as a least-privilege role so a tenant only ever sees its own data. **On by default**; set `FIDES_RLS_ENABLED=false` to disable. It needs a non-superuser database role to do anything, because a superuser ignores every policy, and the server refuses to start when that is the case rather than claim an isolation it does not have.
