...
import { GitBranch, Rocket, ListChecks, ShieldCheck, CheckCircle2 } from "lucide-react";
  ...
  return (
    <div>
      ...
      <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <Card label="Flows" value={flows === null ? "…" : String(flows)} icon={GitBranch} />
        <Card label="Deployments" value={dora ? String(dora.deployments) : "…"} sub={dora ? `${dora.deployment_frequency_per_day.toFixed(2)}/day` : undefined} icon={Rocket} />
        <Card label="Trails" value={dora ? String(dora.trails) : "…"} icon={ListChecks} />
        <Card
            ...
            <table className="w-full text-left text-xs font-mono">
              ...
              <tbody>{trail.map((a) => (
                <tr key={a.id} className="border-t border-border">
                  <td className="py-1"><span className="flex items-center gap-1.5"><CheckCircle2 className={`size-3.5 ${a.is_compliant ? "text-green-400" : "text-red-400"}`} />{a.name}</span></td>
                  <td>{a.type_name}</td>
